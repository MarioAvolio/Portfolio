...
from indexing.helper import make_prompt
from indexing.splitter import CustomTextSplitter
...
class Indexer:
    ...
    def __init__(self, knowledge_base, db_name, embeddings) -> None:
        self._knowledge_base = knowledge_base  # ".md" files
        self._db_name = db_name  # Chroma db name
    ...
    def _create_chunks(self, documents):
        """
        Create chunks using an LLM model
        """
    ...
    def _create_embeddings(self, chunks):
        if os.path.exists(self._db_name):
            Chroma(
                persist_directory=self._db_name, embedding_function=self._embeddings
            ).delete_collection()  # remove previous databases
    ...
    def get_vector_store(self):
        documents = self._fetch_documents()[
            :10
        ]  # TODO: remove this before running, limit to 10 documents for testing
