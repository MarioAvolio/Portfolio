...
class RagUnavailableError(ServiceError):
    """Raised when the RAG pipeline cannot be initialised or queried.

    Typical causes: a missing ``OPENAI_API_KEY``, missing ML dependencies, or a
    failure building the local vector store.
    ...
    message = "RAG pipeline unavailable (check OPENAI_API_KEY / dependencies / index build)."
